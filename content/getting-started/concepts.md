...
All apps need to use a "buildpack" specific to their language, which sets up dependencies for their language stack. There are [standard buildpacks for most languages](https://docs.cloudfoundry.org/buildpacks/), and we strongly encourage you to use the standard buildpacks. cloud.gov supports these standard buildpacks and provides security updates for them.

Buildpacks will usually be auto-detected when you set up an app. In the rare case where the buildpack does not get detected correctly, you can specify it in the manifest (as below) or with the `-b` flag. To reference it, use either the buildpack name:
