...
These service plans are available in [sandbox spaces]({{< relref "overview/pricing/free-limited-sandbox.md#sandbox-limitations" >}}).

For prototyping and production orgs, these service plans are currently no cost but will later have pricing per GB per month. [Learn about managed service pricing.]({{< relref "overview/pricing/managed-services-cost.md" >}})
...
#### Putting and getting an object with the cloud.gov managed S3 bucket:

Here's an example for the (unlikely) case of using the `aws s3` command-line tools to put the `s3.md` file from a developer system and then access it from within a cloud.gov application:

Set up the bucket and keys for developer access:

```sh
cf create-service s3 basic s3bucket
cf create-service-key s3bucket s3bucket-key
```

From a developer system, set the AWS environment variables and put the object in the bucket:

```sh
s3creds=$(cf service-key s3bucket s3bucket-key | grep -v '^Getting')
AWS_ACCESS_KEY_ID=$(echo $s3creds | jq -r '.access_key_id')
AWS_SECRET_ACCESS_KEY=$(echo $s3creds | jq -r '.secret_access_key')
AWS_DEFAULT_REGION=$(echo $s3creds | jq -r '.region')
bucket=$(echo $s3creds | jq -r '.bucket')
export AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_DEFAULT_REGION

aws s3 cp s3.md s3://$bucket
aws s3 ls s3://$bucket
```

Bind the S3 bucket service to an `example-app` which has `aws` CLI installed, e.g., by addng `awscli` to `requirements.txt` for a python-based application:

```sh
cf bind-service example-app s3bucket
```

Then in `example-app` you could have code like this:

```sh
s3creds=$(echo $VCAP_SERVICES | jq -r '.s3[0].credentials')
...
aws s3 cp s3://$bucket/s3.md s3.md
```

#### Example using bucket provisioned outside of cloud.gov

Typically, services external to cloud.gov are provided to applications via a [user-provided service](https://docs.cloudfoundry.org/devguide/services/user-provided.html). For an S3 bucket, you can store the credentials, temporarily, in a JSON file. E.g.:

s3.json:

```javascript
{ 
    "access_key_id": "AKIAKAOIAF98HAWE09AW",
    "secret_access_key": "Casldfjalksdfalsasfdjz",
    "region": "us-west-1",
    "bucket": "my-cg-demo-bucket"
}
```

Then create the user-provided service, and bind it:

```sh
cf create-user-provided-service s3bucket-external -p s3.json
cf bind-service example-app s3bucket-external
```

Your code on the application side is almost identical, except for determining `s3creds`:

```sh
s3creds=$(echo $VCAP_SERVICES | jq -r '.["user-provided"][0].credentials')
...
aws s3 cp s3://$bucket/s3_md s3_md
```
