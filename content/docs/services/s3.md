...
`basic-sandbox` | Similar plan as `basic`, but bucket is deleted when `cf delete-service <SERVICE-INSTANCE-NAME>` is executed | Will be paid per GB per month
`basic-public-sandbox` | Similar plan as `basic-public`, but bucket is deleted when `cf delete-service <SERVICE-INSTANCE-NAME>` is executed | Will be paid per GB per month
