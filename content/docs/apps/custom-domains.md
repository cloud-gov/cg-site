...
The [Manual Method](#manual-method) is available to all users. We've also developed a [Managed Service Method](#managed-service-method), but this is not automatically available to most users yet because it's in cloud.gov's GovCloud environment (if you need this, [let us know](/help/)). Both options support IPv6.
